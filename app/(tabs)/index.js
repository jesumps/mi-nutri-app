...
export default function App() {
  ...
  const cargarTodo = async () => {
    try {
      ...
      if (met) setMetas(JSON.parse(met) || METAS_INICIALES);
  ...
  const modificarDato = (id, cambio) => {
    ...
    const valorActual = nuevoRegistro[diaSeleccionado][id] || 0;
    nuevoRegistro[diaSeleccionado][id] = Math.max(0, valorActual + cambio);
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* SECCIÓN DE MODALES (FUERA DEL SCROLL PARA EVITAR BLOQUEOS) */}
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{flex:1}}>
        <LinearGradient colors={['#1e293b', '#0f172a']} style={styles.header}>
          <View style={styles.headerRow}>
            ...
            <TouchableOpacity 
              onPress={() => { console.log("Click ajustes"); setModalConfig(true); }} 
          ...
          <View style={styles.statsCard}>
            <View><Text style={styles.statsLabel}>Adherencia Mensual</Text><Text style={styles.statsValue}>{calcularAdherencia().toFixed(1)}%</Text></View>
            <Progress.Circle size={50} progress={calcularAdherencia()/100} color="#10b981" unfilledColor="rgba(255,255,255,0.1)" borderWidth={0} thickness={5} />
          </View>
        </LinearGradient>

        <ScrollView style={styles.content} pointerEvents="auto">
          ...
          {verCalendario && (
            <Calendar onDayPress={day => {setDiaSeleccionado(day.dateString); setVerCalendario(false);}} markedDates={{[diaSeleccionado]: {selected: true, selectedColor: '#3b82f6'}}} style={styles.calendarCard}/>
          )}
...
const styles = StyleSheet.create({
  ...
  settingsBtn: { backgroundColor: 'rgba(255,255,255,0.2)', padding: 12, borderRadius: 15, width: 55, height: 55, justifyContent: 'center', alignItems: 'center' },
  statsCard: { backgroundColor: 'rgba(255,255,255,0.1)', marginTop: 15, padding: 15, borderRadius: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  stepBtn: { backgroundColor: '#f1f5f9', width: 40, height: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginLeft: 5 },
  ...
  cancelText: { textAlign: 'center', color: '#ef4444' }
});